...
async def fetch_and_save_history(session: aiohttp.ClientSession, ticker: str, start: str = None, end: str = None):
    params = {}
    if start:
        params['from'] = start
    if end:
...
async def main():
    async with aiohttp.ClientSession() as session:
        ...
        tasks = [fetch_and_save_history(session, ticker, start="2025-01-01", end="2025-12-14") for ticker in tickers]
